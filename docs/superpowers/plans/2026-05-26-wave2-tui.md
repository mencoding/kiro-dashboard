...
**Goal:** TUI interativa com 6 abas (Now / Today / Projects / Models / Tools / Session) navegáveis por atalhos numéricos + setas, alavancando todo o agregador existente. **Auto-refresh seletivo**: Now atualiza a cada 2s (`set_interval`); demais abas são snapshot ao montar + tecla `r` para refrescar manualmente.
...
**Decisão sobre auto-refresh** (alinhada ao padrão atual do `claude-dash` em produção):

| Aba | Auto-refresh | Custo de re-render | Justificativa |
|---|---|---|---|
| **Now** | sim (`NOW_REFRESH_SEC = 2.0`) | leve (~9 `.json` pequenos) | É a aba "live" |
| **Today** | não | pesado (re-agrega todas as sessões do dia) | Manual `r` quando necessário |
| **Projects** | não | pesado | Manual |
| **Models** | não | pesado | Manual |
| **Tools** | não | muito pesado (relê `.jsonl` de megabytes) | Manual |
| **Session** | não | pesado | Manual |

A constante `NOW_REFRESH_SEC = 2.0` espelha o valor já em produção no `claude-dash` para padronização entre os dois painéis.
...
class NowTab(Container):
    ...
    A constante de intervalo espelha o ``NOW_REFRESH_SEC = 2.0`` do
    ``claude-dash`` em produção. Apenas esta aba tem auto-refresh; as
    demais são snapshot manual.
    """
    ...
    def on_mount(self) -> None:
        ...
        # Auto-refresh apenas desta aba (Now é a única "live").
...
- [ ] Auto-refresh **apenas** na NowTab (`set_interval` com `NOW_REFRESH_SEC = 2.0`)
- [ ] Today/Projects/Models/Tools/Session **sem** `set_interval` — manual via `r`
- [ ] Cada aba expõe `refresh_snapshot()` (recompoe sob demanda)
- [ ] `r` invoca `refresh_snapshot` da aba ativa (override manual mesmo na Now)
...
- [ ] README documenta a estratégia de auto-refresh seletivo
